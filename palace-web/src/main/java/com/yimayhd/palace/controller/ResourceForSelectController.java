...
public class ResourceForSelectController extends BaseController {
	...
	@Autowired
	private SockpuppetService sockpuppetService;
	@Autowired
	 ...
	 * author:xmn
	 */
	@RequestMapping(value = "/querySockpuppetForSelect")
	public @ResponseBody ResponseVo querySockpuppetForSelect(SockpuppetListQuery query, Integer pageNumber) {
		try {
			if (pageNumber != null) {
				query.setPageNo(pageNumber);
			}
			PageVO<SockpuppetVO> pageVo = sockpuppetService.getUserListByPage(query);
			Map<String, Object> result = new HashMap<String, Object>();
			result.put("pageVo", pageVo);
			result.put("query", query);
			return new ResponseVo(result);
